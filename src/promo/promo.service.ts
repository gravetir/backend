...
export class PromoService {
  ...
  async create(dto: CreatePromoDto, image: Express.Multer.File) {
    return this.repository.save({
      ...
      typeEngine: dto.typeEngine,
      workVoltage: dto.workVoltage,
    });
  }
  ...
  async update(id: number, dto: UpdatePromoDto, image: Express.Multer.File) {
    ...
    if (dto.driveUnit) {
      toUpdate.driveUnit = dto.driveUnit;
    }
    if (dto.typeEngine) {
      toUpdate.typeEngine = dto.typeEngine;
    }
    if (dto.workVoltage) {
      toUpdate.workVoltage = dto.workVoltage;
    }
    if (dto.title) {
